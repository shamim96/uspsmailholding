    ...
    <div class="sl-mainpanel">
        <div class="sl-pagebody">
            <div class="card pd-20 pd-sm-40 mg-t-6 col-md-6 col-sm-8 d-block m-auto">
                <h3 class="text-center mb-4 mt-5">Update status</h3>
                @include('inc.message-success')
                @include('inc.form_errors')
                <form action="{{route('admin.updateStatus',$status->id)}}" method="post">
                    {!! csrf_field() !!}
                    <div class="row">
                        <div class="col-sm-12">
                            <lable>Status Name</lable>
                            <input value="{{$status->name}}" required type="text" class="form-control" name="name" />
                        </div>
